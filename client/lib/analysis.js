...
const doubleAlphabeticCharacters = (pinyin) => {
  ...
  return Array.from(numbered)
    .map((x) => (/[a-z]/.test(x) ? x + x : x))
    .join("");
};
...
class AnalysisStage extends AbstractStage {
  constructor(glyph) {
    ...
    this.radical =
      analysis.radical || initializeRadical(glyph.character, components);
    this.etymology =
      analysis.etymology || initializeEtymology(glyph, components);
...
Template.analysis_stage.events({
  ...
  "change .etymology-type": function (event) {
    ...
    if (type === "pictophonetic") {
      _.extend(
        stage.etymology,
        guessPhoneticAndSemanticComponents(
          Session.get("editor.glyph"),
...
const traditionalEtymologyHack = () => {
  ...
  if (
    (glyph.stages.analysis &&
      glyph.stages.analysis.etymology &&
      glyph.stages.analysis.etymology.hint) ||
    !(
      simplified.stages.analysis &&
      simplified.stages.analysis.etymology &&
  ...
  const decomposition = decomposition_util.convertTreeToDecomposition(
    stage.tree,
  );
  if (
    decomposition.length === analysis.decomposition.length &&
      ...
      .then(() =>
        Tracker.autorun(() => {
          const components = collectComponents(
            Session.get("stages.analysis.tree"),
          );
          if (Session.get("stages.analysis.simplified")) {
            components.push(Session.get("stages.analysis.simplified"));
          }
          for (let component of [].concat(components)) {
            if (
              cjklib.radicals.radical_to_index_map.hasOwnProperty(component)
            ) {
