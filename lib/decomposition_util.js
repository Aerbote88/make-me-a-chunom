...
const parseSubtree = (decomposition, index) => {
  ...
  // Use codePointAt + fromCodePoint so SIP characters (CJK Ext B and up,
  // encoded as UTF-16 surrogate pairs) are treated as a single component,
  // not two. Advance by current.length — 1 for BMP, 2 for SIP.
  const current = String.fromCodePoint(decomposition.codePointAt(index[0]));
  index[0] += current.length;
