...
const MAX_BRIDGE_DISTANCE = 96;
const MIN_CORNER_ANGLE = 0.05 * Math.PI;
const MIN_CORNER_TANGENT_DISTANCE = 4;
const REVERSAL_PENALTY = 0.5;
const MAX_SEGMENT_LENGTH = 120;

// Subdivide long path segments to create more endpoint dots.
// Uses De Casteljau splitting at t=0.5 for Bezier curves.
const subdividePaths = (paths) => {
  let changed = true;
  while (changed) {
    changed = false;
    for (let i = 0; i < paths.length; i++) {
      ...
      for (let j = 0; j < paths[i].length; j++) {
        const seg = paths[i][j];
        const dist = Math.sqrt(Point.distance2(seg.start, seg.end));
        if (dist > MAX_SEGMENT_LENGTH) {
          changed = true;
          const t = 0.5;
          if (seg.control !== undefined) {
            // Quadratic Bezier: De Casteljau split at t=0.5
            const ctrl1 = Point.add(
              Point.scale(seg.start, 1 - t),
              Point.scale(seg.control, t),
            );
            const ctrl2 = Point.add(
              Point.scale(seg.control, 1 - t),
              Point.scale(seg.end, t),
            );
            const mid = Point.add(
              Point.scale(ctrl1, 1 - t),
              Point.scale(ctrl2, t),
            );
            newPath.push({ start: seg.start, control: ctrl1, end: mid });
            newPath.push({ start: mid, control: ctrl2, end: seg.end });
          } else {
            // Linear segment: split at midpoint
            const mid = Point.midpoint(seg.start, seg.end);
            newPath.push({ start: seg.start, end: mid, control: undefined });
            newPath.push({ start: mid, end: seg.end, control: undefined });
          }
        } else {
          newPath.push(seg);
        }
...
function Endpoint(paths, index) {
  ...
  if (
    this.segments[0].control !== undefined &&
  ...
  if (
    this.segments[1].control !== undefined &&
...
const extractStroke = (
  ...
  const advance = (index) => [
    index[0],
    (index[1] + 1) % paths[index[0]].length,
  ];

  const angle = (index1, index2) => {
    const diff = Point.subtract(
      endpoint_map[Point.key(index2)].point,
  ...
  while (true) {
    ...
    if (bridge_adjacency.hasOwnProperty(key)) {
      ...
      // When skip_dead_bridges is set, don't follow bridges that lead to
      // already-extracted or visited segments (unless it completes the loop).
      // Instead, continue along the current path outline.
      const next_key_check = Point.key(next);
      if (
        skip_dead_bridges &&
        !Point.equal(next, start) &&
        (extracted_indices[next_key_check] || visited[next_key_check])
      ) {
        // Skip this bridge - continue along the path outline
      } else {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    for (var i = 0; i < paths.length; i++) {
      for (var j = 0; j < paths[i].length; j++) {
        ...
        const skip_dead_bridges = attempt >= 3;
        const stroke = extractStroke(
          paths,
          endpoint_map,
          bridge_adjacency,
          log,
...
stroke_extractor.getBridges = (path, classifier) => {
  const paths = subdividePaths(svg.convertSVGPathToPaths(path));
  const endpoints = [];
  for (let i = 0; i < paths.length; i++) {
    for (let j = 0; j < paths[i].length; j++) {
...
export { stroke_extractor, Endpoint };
